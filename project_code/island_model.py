...
toolbox.register("migrate",tools.migRing, k=int(population*0.05), selection=tools.selBest)
toolbox.register("algorithm", algorithms.eaSimple, toolbox=toolbox,
                 cxpb=crossover_percent,mutpb=mutation_percent,ngen=gen_frequency, stats=mstats,halloffame=hof,verbose=True)

toolbox.decorate("mate", gp.staticLimit(key=operator.attrgetter("height"), max_value=17))
toolbox.decorate("mutate", gp.staticLimit(key=operator.attrgetter("height"), max_value=17))

def check_best(best,data,answers):
    fit = 0
    for i in range(len(data)):
        value = best(*data[i][:columns])
        fit += hit_check(value,answers[i])
    fit = fit / len(data)
    return fit
...
def main():
    ...
    logs = []
    for i in range(0,generations,gen_frequency):
        results = toolbox.map(toolbox.algorithm, islands)
        temp = [(island,logbook) for island, logbook in results]
        islands = [temp[i][0] for i in range(5)]
        logs.append([temp[i][1] for i in range(5)])
    ...
    for i in range(len(islands)):
        for chromosome in islands[i]:
            tree = toolbox.compile(expr=chromosome)
            temp = check_best(tree,training_data,training_answers)
            if temp > best:
                best_island = i
                best = temp
                best_individual = chromosome
    
    print(f"Check best of run: {check_best(toolbox.compile(expr=best_individual),testing_data,testing_answers)}")
    print(f"Best island is: {best_island+1}")
    ...
    best_answer = f"\n Best Answer of run: {best_individual}\n Best Island of run: {best_island+1}"
    tested_fit = f"\n Fitness on testing data: {best}"
    print(tested_fit)
    # #print (log)
    fit_max = []
    fit_avg = []
    for i in range(generations//gen_frequency):
        for j in range(gen_frequency):
            temp_fit_max = logs[i][best_island].chapters["fitness"].select("max")[j]
            fit_max.append(temp_fit_max)
            temp_fit_avg = logs[i][best_island].chapters["fitness"].select("avg")[j]
            fit_avg.append(temp_fit_avg)
    ...
    data_file.write(str(best))
    data_file.write(best_answer)
    data_file.close()
