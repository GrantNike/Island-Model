...
gens = 50
runs = 10
gen = [i for i in range(gens+1)]

avg_max_test1 = numpy.array([0 for i in range(gens+1)])
avg_test1 = numpy.array([0 for i in range(gens+1)])
testing_avg1 = 0
...
avg_max_test2 = numpy.array([0 for i in range(gens+1)])
avg_test2 = numpy.array([0 for i in range(gens+1)])
testing_avg2 = 0
for run in range(1,11):
...
#third
avg_max_test3 = numpy.array([0 for i in range(gens + 1)])
avg_test3 = numpy.array([0 for i in range(gens + 1)])
testing_avg3 = 0
for run in range(1, 11):
    input = open(f"output3/output_run{run}.txt")
    ...
    for i in range(len(min_fit)):
        min_fit[i] = float(min_fit[i].strip("]"))
    avg_max_test3 = avg_max_test3 + min_fit
    ...
    for i in range(len(avg_fit)):
        avg_fit[i] = float(avg_fit[i].strip("]"))
    avg_test3 = avg_test3 + avg_fit

    testing_avg3 += float(input.readline().strip('\n'))

    input.close()
avg_max_test3 = avg_max_test3 / runs
avg_test3 = avg_test3 / runs
testing_avg3 = testing_avg3 / runs
...
line3 = ax1.plot(gen, avg_max_test3, "g-", label="Max Fitness Test 3")
for tl in ax1.get_yticklabels():
    tl.set_color("g")
...
line3 = ax1.plot(gen, avg_test3, "g-", label="Average Fitness Test 3")
for tl in ax1.get_yticklabels():
    tl.set_color("g")
...
output.write(f"Average fitness on testing data for parameters3: {testing_avg3}\n")
output.write("\n")
...
for elem in avg_max_test3:
    output.write(str(elem)+"\n")
output.write("avg_max_test3"+"\n")
...
for elem in avg_test3:
    output.write(str(elem)+"\n")
output.write("avg_test3"+"\n")
